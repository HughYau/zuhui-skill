...
## 🎭 个性化导师风格 (Advisor Personas)

不同导师对研究进度的关注点往往各不相同。你可以通过设置导师的“性格预设”，自动调整汇报信息的颗粒度、详略重心和口吻：

- 🦅 **放养型 (`hands-off`)**：关注结论与宏观进展。自动提炼核心产出，省略冗长的实验参数，精简扼要。
- 🕵️ **刨根问底型 (`detail-oriented`)**：关注严谨细致。会详细呈现实验设定、对比数据链条、参数选择原因，甚至失败尝试的推理。
- 🔭 **高屋建瓴型 (`high-level`)**：关注研究大局。适度弱化操作细节，侧重展现进展的学术意义、核心 Insight 与整体方向。
- 💡 **直觉比喻型 (`intuitive`)**：关注概念的直观理解。善于使用生动的比喻来解释复杂的算法或现象，降低沟通门槛。
- 🛠️ **完全自定义**：除了内置预设，你也完全可以在配置文件中通过特定的要求，量身定制只属于你的特殊汇报风格。

告别千篇一律的 AI 套话流水账，用最契合对方思维习惯的语言进行高效科研沟通。✨
